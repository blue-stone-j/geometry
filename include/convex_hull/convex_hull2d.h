...
class ConvexHull2D
{
 public:
  ...
  bool GetConvexHullWithoutGroundAndHead(const CLOUD_IN_TYPE &in_cloud, const float &distance_above_ground_thres,
...
bool ConvexHull2D<CLOUD_IN_TYPE, CLOUD_OUT_TYPE>::MockConvexHull(CLOUD_OUT_TYPE *out_polygon)
{
  ...
  Eigen::Matrix<double, 2, 1> maxv;
  Eigen::Matrix<double, 2, 1> minv;
  maxv << in_cloud_->at(0).x, in_cloud_->at(0).y;
  minv = maxv;
